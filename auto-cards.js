 ...
 *
 * Moteur de découverte partagé (discover) réutilisable par les autres cartes
 * de la série : énergie, piles, santé, accès.
 */
...
/**
 * Résout la pièce d'une entité : registre d'entités d'abord, puis appareil.
 */
 ...
 * @param {object} hass
 * @param {object} opts
 *   domains       : ['sensor'] — domaines acceptés
 *   deviceClasses : ['temperature'] — device_class acceptées
 *   units         : ['W'] — unités acceptées (alternative aux device_class)
 *   includeDiagnostic : false — inclure entity_category diagnostic/config
 *   includeHidden : false
 *   exclude       : ['ballon'] — motifs sur entity_id et nom
 *   include       : ['sensor.x'] — entités forcées
 *   areas         : ['salon'] — restreint à ces pièces (id ou nom)
 *   requireNumeric: true
 */
...
const I = {
  hum: `<path d="M12 3s6 6.4 6 10a6 6 0 1 1-12 0c0-3.6 6-10 6-10z"/>`,
};
...
class ComfortCard extends HTMLElement {
  ...
  setConfig(config) {
    this._config = {
      ...
      multiple: "average", // average | first | list
      max_rows: 0, // 0 = illimité
      humidity_low: 35,
      humidity_high: 60,
      humidity_very_high: 70,
      temp_low: 17,
      temp_high: 26,
      outdoor: null, // auto : entité weather ou pièce « extérieur »
  ...
  _verdict(t, h) {
    ...
    let level = "ok";
    if (h != null) {
      if (h >= c.humidity_very_high) {
        score += 3;
        label = "Trop humide";
        level = "warn";
      } else if (h >= c.humidity_high) {
        score += 2;
        label = "Humide";
        level = "warn";
      } else if (h <= c.humidity_low - 15) {
        score += 2;
        label = "Sec";
        level = "warn";
      } else if (h <= c.humidity_low) {
        score += 1;
        label = "Un peu sec";
        level = "warn";
      }
    ...
    if (t != null) {
      if (t >= c.temp_high + 4) {
        ...
        label = "Trop chaud";
        level = "warn";
      } else if (t >= c.temp_high) {
        score += 1;
        if (level === "ok") label = "Chaud";
      } else if (t <= c.temp_low - 3) {
        score += 3;
        label = "Trop froid";
        level = "warn";
      } else if (t <= c.temp_low) {
        score += 1;
        if (level === "ok") label = "Frais";
      }
    }
  ...
  _build() {
    this.shadowRoot.innerHTML = `<style>${ComfortCard.styles}</style>
      <ha-card>
        <div class="ch">
          <div class="ci"><svg viewBox="0 0 24 24">${I.hum}</svg></div>
  ...
  _update() {
    ...
    let rows = this._collect();
    const warn = rows.filter((r) => r.level === "warn").length;

    /* Signature pour éviter les re-rendres inutiles */
    const sig = rows
...
ComfortCard.styles = `
:host{--cf-warn:#ffc76b;--cf-ok:#8fbfae;display:block;}
...
.ci{width:34px;height:34px;border-radius:11px;flex-shrink:0;
  background:rgba(143,176,201,.10);border:1px solid rgba(143,176,201,.24);
...
.cc.warn{background:rgba(255,199,107,.12);border-color:rgba(255,199,107,.3);color:var(--cf-warn);}
.cc.ok{background:rgba(143,191,174,.12);border-color:rgba(143,191,174,.3);color:var(--cf-ok);}
...
.rhd{display:flex;gap:8px;margin:15px 0 4px;font-size:8px;letter-spacing:1px;
  text-transform:uppercase;color:rgba(255,255,255,.3);font-weight:600;}
...
.rr{display:flex;align-items:center;gap:8px;padding:9px 0;cursor:pointer;
  border-bottom:1px solid rgba(255,255,255,.05);}
.rr:last-child{border-bottom:none;}
.rr:hover{background:rgba(255,255,255,.02);}
.rn{flex:1;font-size:12px;color:rgba(255,255,255,.75);min-width:0;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.rn i{font-style:normal;font-size:8.5px;color:rgba(255,255,255,.28);margin-left:6px;}
.rt,.rh{font-size:12.5px;font-weight:600;text-align:right;font-variant-numeric:tabular-nums;}
.rt{width:48px;} .rh{width:46px;color:#8fb0c9;}
...
window.customCards.push({
  ...
  preview: false,
  documentationURL: "https://github.com/votre-compte/ha-auto-cards",
});
...
class ComfortCardEditor extends HTMLElement {
  ...
  _field(label, field, type = "text", value, placeholder = "") {
